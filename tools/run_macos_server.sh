#!/usr/bin/env bash
#
# run_macos_server.sh — One-key macOS launcher for qwen3asr_cpu (server + optional HTTPS proxy).
#
# 用法:
#   tools/run_macos_server.sh                      # 前台 qasr_server (HTTP)
#   tools/run_macos_server.sh --detach             # 后台 qasr_server (HTTP)
#   tools/run_macos_server.sh --detach --https     # 后台 qasr_server + HTTPS proxy (推荐)
#   tools/run_macos_server.sh --stop               # 停 --detach 起的 server 和 proxy
#   tools/run_macos_server.sh --status             # 查 HTTP + HTTPS health
#   tools/run_macos_server.sh --https-info         # 显示 cert / proxy 状态
#
# 必填环境变量:
...
#   想跨重启复用 cert: 设 QASR_TLS_CERT_DIR (例: ~/qasr-tls), 配合 --https
#
# 路径:
#   QASR_PROJECT_ROOT  项目根 (默认自动探测 = 脚本 ../)
#   QASR_BUILD_DIR     编译输出 (默认 $PROJECT_ROOT/build/macos-accelerate)
#   QASR_LOG_FILE      server 日志 (默认 /tmp/qasr_server.log)
#   QASR_PID_FILE      server PID 文件 (默认 /tmp/qasr_server.pid)
#   QASR_PROXY_LOG     proxy  日志 (默认 /tmp/qasr_proxy.log)
...
BUILD_DIR="${QASR_BUILD_DIR:-$PROJECT_ROOT/build/macos-accelerate}"
...
Flags:
  ...
  --verbose           覆盖 \$QASR_VERBOSITY=3 (开发用)
  -h, --help          打印本帮助
...
Env vars (可选):
  ...
  QASR_PROXY_LOG=...  (默认 /tmp/qasr_proxy.log)
  QASR_PROXY_PID=...  (默认 /tmp/qasr_proxy.pid)
  QASR_TLS_CERT_DIR=... 持久化 cert 目录

Examples:
  export QASR_MODEL_DIR=\$HOME/models/Qwen3-ASR-0.6B
  $(basename "$0") --detach --https       # 标准: 后台 + HTTPS
...
check_required() {
    ...
    if [[ -z "$MODEL_DIR" ]]; then
        ...
        log_err "      snapshot_download('Qwen/Qwen3-ASR-0.6B')"
        log_err ""
    ...
    if [[ ! -f "$MODEL_DIR/model.safetensors" && ! -f "$MODEL_DIR/model-00001-of-00002.safetensors" && ! -f "$MODEL_DIR/model-00001-of-00003.safetensors" ]]; then
        log_err "QASR_MODEL_DIR=$MODEL_DIR 不含 model.safetensors 或分片"
        log_err "  请检查路径, 或重新下载"
        exit 1
    fi
    ...
    if [[ -n "$REALTIME_MODEL_DIR" ]]; then
        ...
        if [[ ! -f "$REALTIME_MODEL_DIR/model.safetensors" && ! -f "$REALTIME_MODEL_DIR/model-00001-of-00002.safetensors" ]]; then
    ...
    if [[ ! -x "$BUILD_DIR/qasr_server" ]]; then
        ...
        log_err "  请先跑: tools/build_macos.sh"
        exit 1
    fi
...
do_stop() {
    ...
    # 兜底
    pkill -9 -f "build/macos-accelerate/qasr_server" 2>/dev/null || true
...
do_status() {
    ...
    for attempt in 1 2 3; do
        code="$(curl -s --max-time 3 -o /dev/null -w '%{http_code}' "$http" 2>/dev/null)" || code="000"
    ...
    if [[ -f "$PROXY_PID" ]] && kill -0 "$(cat "$PROXY_PID" 2>/dev/null)" 2>/dev/null; then
        ...
        code=""
        for attempt in 1 2 3; do
            code="$(curl -sk --max-time 3 -o /dev/null -w '%{http_code}' "$https" 2>/dev/null)" || code="000"
            if [[ "$code" == "200" ]]; then break; fi
            sleep 1
        done
        if [[ "$code" == "200" ]]; then
...
check_port_free() {
    ...
    if command -v lsof >/dev/null 2>&1; then
        if lsof -iTCP:"$port" -sTCP:LISTEN -P -n 2>/dev/null | grep -q ":$port"; then
            ...
            log_err "        或跑本脚本 --stop"
            log_err "        或换端口:    QASR_PORT=19993 tools/run_macos_server.sh --detach --https"
            return 1
        fi
    elif command -v netstat >/dev/null 2>&1; then
        if netstat -an -p tcp 2>/dev/null | grep -q "LISTEN.*:$port"; then
            log_err "$label 端口 $port 已被占用"
            return 1
        fi
    fi
    return 0
